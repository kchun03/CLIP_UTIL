...
public class reportinfoControl
{
	...
	public void readReport() throws Exception {
	    ...
	    try
	    {
	      reportFile = Rexpert4.read(this.rptName);
	...
	private void findLabel(ControlLabel control, String sControlPath) {
		if(control.getOutputFormat() != null && control.getOutputFormat() != "") {
			...
			/*
			System.out.println("========================================");
			System.out.println(sControlPath + " > " + control.getName() +" (라벨)");
			System.out.println("========================================");
			System.out.println("-> "+control.getOutputFormat());
			*/
	...
	public void setWritelog(String logFilePath) throws Exception {
		...
		*/
		try {
			...
			out.write("파일명 : "+rptName); out.newLine();
			out.newLine(); 
			for(int i=0; i<list.size(); i++){
				HashMap getMap = new HashMap();
				getMap = (HashMap)list.get(i);
				
				//System.out.println("컨트롤 위치 : "+getMap.get("path").toString());
				//System.out.println("-> "+getMap.get("detailValue").toString());
				out.newLine();
